<h2 align="center">CAD-MOTOTAXISTA - Documentação Técnica</h2>

### 🏍️ Sistema CADMOTOTAXISTA
Sistem Web para gestão de emissão de alvarás de Mototaxistas e Motofretistas da SMTT de Santo Antônio de Jesus

<p align="justify">
O CADMOTOTAXISTA é um sistema desenvolvido para a Secretaria Municipal de Trânsito e Transporte (SMTT) do município de Santo Antônio de Jesus (BA), com o objetivo de gerenciar o cadastro, a regulamentação e a fiscalização de profissionais de transporte individual — mototaxistas e motofretistas.
</p>

<p align="justify">
Os apêndices a seguir reúnem os principais artefatos complementares da documentação técnica do sistema CAD-MOTOTAXISTA. Esses materiais oferecem suporte à compreensão da arquitetura, das tecnologias empregadas, dos processos de validação e da modelagem conceitual e estrutural da aplicação, permitindo uma análise mais aprofundada dos componentes que fundamentam a solução
</p>

---
<!-- Quadro: Estrutura dos Módulos -->
<table border="2" align="center" style="border-collapse: collapse; text-align:center;">
  <thead>
    <tr style="background-color:#2F4F4F; color:white;">
      <th colspan="3">QUADRO DE APÊNDICES</th>
    </tr>
    <tr style="background-color:#708090; color:white;">
      <th align="center">Apêndice</th>
      <th align="center">Descrição</th>
      <th align="center">Link</th>
    </tr>
  </thead>
  <tbody>
    <!-- APÊNDICE C -->
    <tr style="background-color:#2F4F4F; color:white;">
      <th colspan="3">Arquitetura, Tecnologias e Listagens</th>
    </tr>
    <tr align="center">
      <td>APÊNDICE C</td>
      <td>Listagens e versões das tecnologias aplicadas</td>
      <td><a href="https://github.com/Albertinesilva/Cad-mototaxista-Documentacao-Tecnica/blob/main/apendice-c/APENDICE_C.md">Acessar</a></td>
    </tr>
    <!-- APÊNDICE D -->
    <tr style="background-color:#2F4F4F; color:white;">
      <th colspan="3">Evidências de Validação</th>
    </tr>
    <tr align="center">
      <td>APÊNDICE D</td>
      <td>Documentação, testes e validações do sistema</td>
      <td><a href="https://github.com/Albertinesilva/Cad-mototaxista-Documentacao-Tecnica/blob/main/apendice-d/APENDICE_D.md">Acessar</a></td>
    </tr>
    <!-- APÊNDICE E -->
    <tr style="background-color:#2F4F4F; color:white;">
      <th colspan="3">Modelagem e Artefatos de Engenharia</th>
    </tr>
    <tr align="center">
      <td>APÊNDICE E</td>
      <td>Modelagem relacional, DDL e diagramas UML</td>
      <td><a href="https://github.com/Albertinesilva/Cad-mototaxista-Documentacao-Tecnica/blob/main/apendice-e/APENDICE_E.md">Acessar</a></td>
    </tr>
  </tbody>
</table>

---
### 📘 Licença de Uso e Distribuição – Albert Silva de Jesus (v1.0)

Copyright © 2025 **Albert Silva de Jesus**.  
Todos os direitos reservados.

Este software e sua documentação foram desenvolvidos por **Albert Silva de Jesus**.  
A cópia, modificação, distribuição ou redistribuição total ou parcial do código-fonte e/ou da documentação só é permitida mediante **autorização expressa do autor**.

É permitido o uso para **fins acadêmicos, educacionais ou de referência técnica**, desde que seja mantida a devida **citação do autor** e **não haja fins comerciais**.

O autor **não se responsabiliza por danos diretos ou indiretos** resultantes do uso deste software, incluindo falhas, perdas de dados ou prejuízos decorrentes da utilização indevida ou não autorizada do código.

---
| 🌎 LinkedIn                                                              | 👨‍💻 **Autor**                                                                 |
| ------------------------------------------------------------------------ | ---------------------------------------------------------------------------- |
| [LinkedIn](https://www.linkedin.com/in/albert-backend-java-spring-boot/) | [Albert Silva](https://www.linkedin.com/in/albert-backend-java-spring-boot/) |

